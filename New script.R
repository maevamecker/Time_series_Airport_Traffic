...
#Now all our coefficients are significants
...
plot(res3)
    ...
    # WHITE NOISE ASSUMPTION ## ACF of the residuals
...
#Comment:p-value = 0.667 > 0.05 so we accept H0 : white noise assumption -> we haven't miss any important information on our model

  #NORMALITY ASSUMPTION
#Shapito, qqplot, histogram....

#We compute the normalized residuals -> res/std(res), if my residuals are normally distributed, 
#the std residuals should follow the standard normal distribution (95% of chances that my res takes values [-2,2])
#We check this visualy (no relevant outliers)

res_stand<- res3/sqrt(mod3$sigma2)
summary(res_stand)
...
#Function that enables to which obervation correspond the outliers
min(res_stand)
...
#Now we recover the date associated to these traffic outlier
library(zoo)
index(res_stand)[out1]
index(res_stand)[out2]
#The date of the oulier 1 is :  February 2003 
...
# We have two options : change the value to a more plausible one or add a dummy in the model
#We will add a dummy 
...
#so there seems to be a non constant variance, it may be due to the outliers we have identified on the graph of the residuales
...
#Test-> 
library(TSA)
Htest<-McLeod.Li.test(mod3, plot=FALSE)
Htest
...
#We create a new model with our dummy varaible
mod4 <- arima(ltraffic.ts, c(1,1,0), seasonal=list(order=c(0,1,1), period=12), method='ML', xreg = cbind(Paris_Traff$dum_1, Paris_Traff$dum_2))
mod4
...
#The coeff xreg1 is not significant so we remove the first dummy from our model

mod5 <- arima(ltraffic.ts, c(1,1,0), seasonal=list(order=c(0,1,1), period=12), method='ML', xreg = Paris_Traff$dum_2)
mod5
...
#Variance is not constant as all the coeff are not significant
...
plot(cbind(ltraffic.ts,fit1-cb80,fit1+cb80),plot.type='single',lty=c(1,2,2)) #lty=line type

# Proportion of points in the confidence bound
indi <- (ltraffic.ts-(fit1-cb80))>0&(fit1+cb80-ltraffic.ts)>0
prop <- 100*sum(indi)/length(indi)
prop
# If prop >80%, then the fit is considered good. Here it is the case (prop = 85%, the means 82% of the obs lie into the confidence interval)
...
time(traffic.ts)
ts.plot(traffic.ts,xlim=c(2010,2020.500))
lines(2.718^(pred$pred), col="red")
lines(2.718^(pred$pred-1.96*pred$se),col=4,lty=2)
lines(2.718^(pred$pred+1.96*pred$se),col=4,lty=2)


#Here we define txo subsamples and we estimate a model on subsample 1 and 
#then we check the validity of the predictions with respect to subsample 2

data.train <- window(ltraffic.ts,start = c(2000,1), end = c(2013,1))
data.test <- window(ltraffic.ts,start = c(2013,2), end = c(2020,1))

mod.train <- arima(data.train,c(1,1,0), seasonal=list(order=c(0,1,1), period= 12), method='ML')
pred.test <- predict(mod.train, n.ahead = 174)

library(forecast)
accuracy(pred.test$pred, data.test)

#Our RMSE AND MAE are quite low so our model is quite good 
